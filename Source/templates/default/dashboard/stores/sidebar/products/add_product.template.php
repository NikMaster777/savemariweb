    ...
    <div class="col-sm-12">
          ...
          <div id="stage-1" class="stage-wrapper">
            ...
            <script type="text/javascript">
            	$(document).ready(function() {
					...
					$('#add-product').click(function() {
						$.ajax({
							url: '<?php echo DOMAIN; ?>/index.php?page=dashboard&view=store&action=ajax&request=add-product',
							type: 'POST',
							data: 'advert_catid='+ $('#product_catid').val() +'&advert_title='+$('#advert_title').val()+'&advert_price='+$('#advert_price').val()+'&advert_html='+CKEDITOR.instances.advert_html.getData() + '&' + $('#customFields').serialize(),
            ...
            <div class="form-group">
              <label>Title:</label>
              <input type="text" class="form-control" name="" value="" placeholder="Title" id="advert_title"/>
            </div>
				...
				while($fieldData = db::fetch($fieldSQL)) {
						...
						switch($fieldData['field_type']) {
							case 1: { //Textbox
								...
								echo '<input type="text" name="'.$fieldData['field_id'].'" class="form-control" placeholder="'.lib::san($fieldData['field_placeholder']).'" />';
								echo '</div>';
								break;	
							}
							case 2: { //Textarea
								echo '<div class="form-group">';
								echo '<label>'.lib::san($fieldData['field_name']).'</label>';
								echo '<textarea cols="" rows="8" name="'.$fieldData['field_id'].'" class="form-control" placeholder="'.lib::san($fieldData['field_placeholder']).'"></textarea>';
								echo '</div>';
								break;	
							}
							case 3: { //Dropdown
            ...
            <div class="form-group pull-right">
              	<input type="submit" class="btn btn-default" value="Next" name="" id="add-product"/>
            </div>
