...
function ftm_first_run() {
	if ( isset($_GET['firstrun']) )
		do_action( 'ftm_first_run' );
}
add_action( 'admin_notices', 'ftm_first_run', 9999 );
...
function ftm_child_init_image_sizes() {
...
add_action( 'appthemes_first_run', 'ftm_child_init_image_sizes', 11 ); //this hook fires on activation of simply theme
...
function child_setup_new_views_template() {
	require_once dirname( __FILE__ ) . '/includes/ftm-views.php';
	new FTM_User_Dashboard;
	new FTM_Ads_Archive;
...
// Setup New Child Views
/*
function ftm_setup_new_views_template() {
	require_once dirname( __FILE__ ) . '/includes/archive-views.php';
	new FTM_Ads_Archive;

}
add_action( 'appthemes_init', 'ftm_setup_new_views_template' );
*/

// load alternative enqueue file
	require_once( get_stylesheet_directory() . '/includes/ftm-enqueue.php' );
...
if ( ! function_exists( 'ftm_ad_featured_thumbnail' ) ) :
	function ftm_ad_featured_thumbnail() {
		global $post, $cp_options;
...
function ftm_modify_actions() {
	remove_action( 'appthemes_after_post_title', 'cp_ad_loop_meta' );
	add_action( 'appthemes_after_post_title', 'ftm_ad_loop_meta' );
}
add_action( 'appthemes_init', 'ftm_modify_actions' );
...
class AppThemes_Widget_300_Ads extends WP_Widget {
	...
	function AppThemes_Widget_300_Ads() {
		...
		$this->WP_Widget( 'cp_300_ads', __( 'CP 300x250 Ads', CPSR_TD ), $widget_ops, $control_ops );
	}
...
function ftm_widgetsnew_init() {
	register_widget( 'AppThemes_Widget_300_Ads' );

}
add_action( 'widgets_init', 'ftm_widgetsnew_init' );


// correctly load all the jquery scripts so they don't conflict with plugins
if ( ! function_exists( 'ftm_load_scripts' ) ) :
function ftm_load_scripts() {
	global $cp_options;
	...
	wp_enqueue_script( 'theme-scripts', get_stylesheet_directory_uri() . '/includes/js/theme-scripts.js', array( 'jquery' ), '3.3.3', true );
}
endif;
if ( !is_admin() ) {
add_action( 'wp_enqueue_scripts', 'ftm_load_scripts' );
}


// make the password strength meter to work on login template page
function ftm_enqueue_login_scripts() {
    if ( is_page_template( 'tpl-login.php' ) ) {
        wp_enqueue_script( 'utils' );
        wp_enqueue_script( 'user-profile' );
    }
}
add_action( 'wp_enqueue_scripts', 'ftm_enqueue_login_scripts' );
...
function ftm_vienna_mod($postID) {
$thispost = $postID;
	function ftm_get_category_count($input = '' ) {
    global $wpdb;
    if($input == '' ) {
...
$catname = appthemes_get_custom_taxonomy( $thispost, APP_TAX_CAT, 'name' );
	if (isset($cat_id)) {
	if (ftm_get_category_count($cat_id)> 1) {
		//echo "";
		echo "<span class='tabview'>" . __( 'There are ', CPSR_TD ) . ftm_get_category_count($cat_id) . __( ' ads listed under ', CPSR_TD) . $catname . "</span>";
		} else {
		echo "<span class='tabview'>" . __( 'Sorry, there is only ', CPSR_TD ) . ftm_get_category_count($cat_id) . __( ' ad listed under ', CPSR_TD) . $catname . __( ' at this time.', CPSR_TD) . "</span>";
		}
	}
?>
</div>
...
function ftm_sticky_on_top( $sql ){
 ...
 */
function ftm_awesome_stylesheet() {
	
	global $cp_options;
	
	// Respects SSL, Style.css is relative to the current file
	wp_register_style( 'font-awesome', get_stylesheet_directory_uri() . '/css/font-awesome/css/font-awesome.css', false );
	wp_enqueue_style( 'font-awesome' );
	
}
add_action( 'wp_enqueue_scripts', 'ftm_awesome_stylesheet', 100 );


/**
 * Returns user dashboard ad listing actions.
 */
function ftm_get_dashboard_listing_actions( $listing_id = 0 ) {
	global $cp_options;
	...
	if ( $listing_status == 'pending_payment' ) {
		...
		$actions['pay_order'] = array(
			'title' => __( 'Pay now', CPSR_TD ),
			'href' => appthemes_get_order_url( $order->get_id() ),
		);
		if ( $order->get_gateway() ) {
			// reset gateway
			$actions['reset_gateway'] = array(
				'title' => __( 'Reset Gateway', CPSR_TD ),
				'href' => get_the_order_cancel_url( $order->get_id() ),
			);
		}
	}

	return apply_filters( 'ftm_dashboard_listing_actions', $actions, $listing );
}

/**
 * Displays user dashboard ad listing actions.
 */
function ftm_dashboard_listing_actions( $listing_id = 0 ) {
	$listing_id = $listing_id ? $listing_id : get_the_ID();

	$actions = ftm_get_dashboard_listing_actions( $listing_id );
	$li = '';
