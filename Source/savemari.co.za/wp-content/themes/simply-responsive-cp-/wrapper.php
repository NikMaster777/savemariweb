...
<?php if (is_single()) { ?>
    ...
    <meta property="og:image:url" content="<?php if ( $cp_options->ad_images ) cp_get_single_image_url($post->ID, 'thumbnail'); ?>" />
    <meta property="og:image" content="<?php if ( $cp_options->ad_images ) cp_get_single_image_url('noresize', true); ?>" />
    <meta property="og:image" content="http://www.fabtalent.co.uk/wp-content/uploads/2013/04/simply.png" /> <!-- CHANGE PATH TO LINK TO YOU OWN LOGO/IMAGE -->
<?php } ?>

	<!-- CHANGE PATH BELOW TO LINK TO YOU OWN LOGO/IMAGE -->
	<link rel="image_src" href="http://www.fabtalent.co.uk/wp-content/uploads/2013/04/simply.png" />
...
<body <?php body_class(); ?>>
	...
	<div class="container">
		...
		<?php if ( $cp_options->debug_mode ) { ?><div class="debug"><h3><?php _e( 'Debug Mode On', CPSR_TD ); ?></h3><?php print_r( $wp_query->query_vars ); ?></div><?php } ?>

		<?php appthemes_before_header(); ?>
		<?php get_header( app_template_base() ); ?>
		<?php appthemes_after_header(); ?>

		<?php if(function_exists( 'bl_get_template_part' )) {
			bl_get_template_part( 'includes/theme', 'searchbar' );
		} else {
