...
class ACF_Validator {
	...
	protected function default_methods() {
		...
		'height' => array(
			'args' => '0',
			'validate' => 'format',
			'desc' => __( 'Height in feet.', $this->td )
					. '<strong>' . __( 'VALID:', $this->td ) . '</strong> ' . __( '5\'11" or 7\'0"', $this->td ) . '<br />'
					. '<strong>' . __( 'INVALID:', $this->td ) . '</strong> ' . __( '5ft 11in or 145".', $this->td )
		),
		'required' => array(
			'args' => '0',
			'validate' => 'format',
	...
	protected function height( $posted ) {
		if ( !$posted || preg_match( '/^(\d{1,2}\'([0-9]|10|11)\"?$)/i', $posted ) )
			return true;
	}
