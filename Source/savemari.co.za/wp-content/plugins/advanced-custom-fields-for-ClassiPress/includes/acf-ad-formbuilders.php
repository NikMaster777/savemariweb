...
function acf_display_ad_form_fields( $result ) {
	...
	if ( is_page_template( 'tpl-edit-item.php' ) || is_page_template( 'edit-listing.php' ) || is_page_template( 'renew-listing.php' ) )
		$disp_opt = 'edit_ad_display';
	elseif ( is_page_template( 'tpl-add-new.php' || is_page_template( 'create-listing.php' ) ) )
...
function acf_ad_form_fields_args( $args, $result, $post ) {
	...
	if ( ( is_page_template( 'tpl-add-new.php' ) || is_page_template( 'create-listing.php' ) ) && ( ! is_numeric( $post ) ) && !isset( $_POST[ $result->field_name ] ) ) {
...
function acf_listing_validate_fields( $errors ) {
	...
	foreach ( $fields as $field_name => $field ) {
		if ( isset( $_POST[ $field_name ] ) ) {
			$field['value'] = stripslashes_deep( $_POST[ $field_name ] );
		...
		foreach ( $methods as $method ) {
			if ( isset( $field[ $method ] ) && ! $validator->is_valid( $field[ $method ], $posted, $field[ 'limits_attr' ] ) ) {
				...
				$errors = acf_add_listing_error( $errors, $field_name, $message );
			}	
		}
	}
	return $errors;
...
function acf_add_listing_error( $errors, $field_name, $message ) {
	global $wpdb;
	$field = $wpdb->get_row( $wpdb->prepare( "SELECT field_label FROM $wpdb->cp_ad_fields WHERE field_name = %s LIMIT 1", $field_name ) );
	
	if( version_compare( CP_VERSION, '3.4', '<' ) ) {
		$error_prefix = $field->field_label . ' - ';
		$errors[] = '<strong>' . __( 'ERROR', APP_TD ) . '</strong>: ' . $error_prefix . ' ' . $message;
	} else {
		$error_prefix = sprintf( __( 'Error: The "%s" field is invalid.', APP_TD ), translate( $field->field_label, APP_TD ) );
		$errors->add( 'invalid-'. $field_name, $error_prefix . ' ' . $message );
	}
	return $errors;
}
