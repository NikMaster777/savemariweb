...
class DNA_Adfields_Tab extends DNA_Tab {
		...
		<table id="acf_ad_field-table" class="widefat">
			<thead>
				<tr>
					...
					foreach ( $this->properties as $key => $property ) {
						$class = $this->get_table_cell_classes( array_merge( array( 'name' => $key ), $property ), false, 'thead' );
			...
			<tfoot>
				<tr>
					<?php
					foreach ( $this->properties as $key => $property ) {
						$class = $this->get_table_cell_classes( array_merge( array( 'name' => $key ), $property ), false, 'tfoot' );
	...
	private function get_row_actions ( $result ) {
		switch($result->field_perm) {
			...
			?>
				<span class="blur dashicons dashicons-trash"></span>
			<?php
			break;
			default: // regular fields full editing
				// don't change this line to plain html/php. Get t_else error msg
				echo '<a class="deletecross" href="javascript:void(0)" title="' . __( 'Delete', ACF_TD ) . '" id="'. $result->field_id .'"><span class="dashicons dashicons-trash"></span></a>';
	   } // endswitch
	}

	private function get_row_html( $field_name = '', $option_field = '' ) {
		foreach ( $this->properties as $prop_name => $property ) {
			$input['class'] = 'field_' . $prop_name;
			$input['name'] = $field_name . '_' . $prop_name;
			$td_class = ( 0 != $property['col'] ) ? ' class="' . $this->get_table_cell_classes( array_merge( array( 'name' => $prop_name ), $property ), array_merge( array( 'name' => $field_name ), $option_field ), 'tbody' ) . '"' : '';
			if ( $prop_name == 'name' ) {
			...
			$input = apply_filters( 'acf_' . $this->tab . '_field_input', $input, array_merge( array( 'name' => $prop_name ), $property ), array_merge( array( 'name' => $field_name ), $option_field ) );
