...
class DNA_Profile_Tab extends DNA_Tab {
				...
				foreach ( $profile_fields as $key => $profile_field ){ ?>
						<tr data-array_index="<?php echo $count; ?>">
							...
							<td class="row_actions">
								<a href="javascript:void(0)" class="row_remove" title="<?php _e( 'Delete', ACF_TD ); ?>">
	...
	protected function get_row_html( $key = '', $option_field = array(), $row_index = '' ) {
		$properties = $this->properties;
		foreach ( $properties as $prop_name => $property ) {
			$input['class'] = 'field_' . $prop_name;
			$input['name'] = $input['class'] . '_' . $row_index;
			
			$td_class = ( 0 != $property['col'] ) ? ' class="' . $this->get_table_cell_classes( array_merge( array( 'name' => $prop_name ), $property ), array_merge( array( 'name' => $key ), $option_field ), 'tbody' ) . '"' : '';
			...
			$input = apply_filters( 'acf_' . $this->tab . '_field_input', $input, array_merge( array( 'name' => $prop_name ), $property ), array_merge( array( 'name' => $key ), $option_field ) );
