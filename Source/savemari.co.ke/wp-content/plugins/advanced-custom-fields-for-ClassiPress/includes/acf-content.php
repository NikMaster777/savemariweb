...
//add_filter( 'acf_display_profile_field', 'acf_height_filter',10, 3 );
...
add_filter( 'acf_display_ad_field', 'acf_height_filter',10, 3 );
...
function acf_get_loop_fields() {
	if ( !is_archive() && !is_search() && !( is_admin() && DOING_AJAX ) )
		return false;

	global $acf_loop_ad_top, $acf_loop_ad_bottom, $acf_loop_ad_top_p, $acf_loop_ad_bottom_p, $wp_filter, $loop_top_function;
	...
	$loop_top_function = 'cp_ad_loop_meta';
	if( isset( $wp_filter['appthemes_after_post_title'] ) && isset( $wp_filter['appthemes_after_post_title'][10] ) ) {
		reset( $wp_filter['appthemes_after_post_title'][10] );
		$loop_top_function_temp = key( $wp_filter['appthemes_after_post_title'][10] );
		if( $loop_top_function_temp != 'acf_ad_loop_meta_top' ) {
			$loop_top_function = $loop_top_function_temp;
		}
	}
	acf_re_action( 'appthemes_after_post_title', $loop_top_function, 'acf_ad_loop_meta_top' );
}
...
function acf_loop_fields( $option, $acf_u_can_see, $post_id ) {
	...
	foreach ( $$option as $field => $props) {
		...
		$ret .= ' <span class="' . esc_attr( $disp_opt ) . ' ' . esc_attr( $field ) . '">';	
		$ret .= version_compare( CP_VERSION, '3.5', '<' ) ? '<span class="loop-field-separator">&nbsp; | &nbsp;</span>' : '<span class="dashicons dashicons-arrow-right-alt2"></span>';
...
function acf_loop_blocks( $location = 'top' ) {
	...
	global $post, $current_user, $cp_options, $loop_top_function;
	...
	$action( $post );
	$acf_blocks = ob_get_clean();

	if ( 'top' === $location ) {
		if( $loop_top_function && $loop_top_function != __FUNCTION__ && function_exists( $loop_top_function ) ) {
			ob_start();
				call_user_func_array( $loop_top_function, array() );
			$loop_top_output = ob_get_clean();
			//$blocks = preg_match_all( '/(.+)<\/span>(.+)/i', $loop_top_output );
			preg_match_all( '/([\s\S]+<\/span>)([\s\S]+)/i', $loop_top_output, $blocks );
			echo $blocks[1][0] . $acf_blocks . $blocks[2][0];
			
		} else {
			echo '<p class="post-meta">';
			$display_cat = '<span class="dashicons-before folder">';
			if ( get_the_category() )
			...
			echo $display_cat;
			?>
			<span class="dashicons-before owner">
				<?php if ( $cp_options->ad_gravatar_thumb || get_option( 'cp_ad_gravatar_thumb' ) == 'yes' ) appthemes_get_profile_pic( get_the_author_meta( 'ID' ), get_the_author_meta( 'user_email' ), 32 ) ?>
					<?php if ( !is_author() ) the_author_posts_link(); ?>
			</span> |
			<span class="dashicons-before clock">
				<span>
