...
function acf_enqueue_css() {
	...
	if ( is_page_template( 'tpl-add-new.php' ) || is_page_template( 'create-listing.php' ) || is_page_template( 'tpl-profile.php' ) || is_page_template( 'tpl-edit-item.php' ) || is_page_template( 'edit-listing.php' ) || is_page_template( 'renew-listing.php' ) || is_page_template( 'tpl-registration.php' ) || is_page_template( 'tpl-login.php' ) ) {
		...
		wp_enqueue_style( 'acf_loop_style', ACF_URL . '/css/acf-loop.css', 99 );
	}
}
...
function acf_enqueue_js() {
	...
	if ( is_page_template( 'tpl-add-new.php' ) || is_page_template( 'create-listing.php' ) || is_page_template( 'tpl-profile.php' ) || is_page_template( 'tpl-edit-item.php' ) || is_page_template( 'edit-listing.php' ) || is_page_template( 'renew-listing.php' ) || is_page_template( 'tpl-registration.php' ) || is_page_template( 'tpl-login.php' ) ) {
		...
		if ( 'yes' === $date_obj['icon_trigger'] ) {
			if( version_compare( CP_VERSION, '3.5', '<' ) ) {
				$date_obj['buttonImage'] = get_template_directory_uri() . '/images/calendar.gif';
			} else {
				$date_obj['buttonText'] = '<span class="dashicons dashicons-calendar-alt"></span>';
				$date_obj['buttonImageOnly'] = false;
			}
		}
