...
class DNA_Tab {
	...
	protected function get_table_cell_classes( $property, $data = false, $table_location = 'tbody' ) {
		$classes = array( 'col' );
		if( isset( $property['col'] ) && 0 != $property['col'] ) {
			$classes = array( 'dna-col', 'col' . $property['col'] );
		}
		$classes = apply_filters( 'acf_' . $this->tab . '_table_cell_class', $classes, $property, $data, $table_location );
